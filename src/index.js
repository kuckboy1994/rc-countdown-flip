...
const getTimeText = (format = "hh:mm:ss", leftTime) => {
...
export default function({
  option: { leftSecond, format, end, style, standard }
}) {
  const [{ times, leftTime }, setLeftTime] = useState(
    getTimeText(format, leftSecond)
  );
  ...
  return (
    <div className={styles["count-down"]} style={sf}>
      {times.map((item, index) => (
        <Flipper key={index} now={item} style={style} />
      ))}
    </div>
  );
}
